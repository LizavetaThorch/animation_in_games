...
namespace ImGui
{
    ...
    bool ComboWithFilter(const char* label, int* current_item, const std::vector<std::string>& items)
    {
        ...
        ImVec2 pos = ImVec2(CursorPos[0] + expected_w-sz, CursorPos[1]);
        const ImRect bb(pos, ImVec2(pos[0] + size[0], pos[1] + size[1]));
        ...
        RenderArrow(window->DrawList,
         ImVec2(bb.Min[0] + ImMax(0.0f, (size.x - g.FontSize) * 0.5f), bb.Min[1] +  ImMax(0.0f, (size.y - g.FontSize) * 0.5f)), text_col, ImGuiDir_Down);
